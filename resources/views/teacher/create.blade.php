...
<div class="tbl-form-sec mb-5 mt-4">
			<div class="upld-blk-rec">
				...
				<form class="mt-4" id="add_prog_form" method="post" action="{{ route('teacher') }}" enctype="multipart/form-data">
        {{csrf_field()}}
        ...
        <button type="submit" id="add_new_prog">Add New Faculty</button>
		<br> <br>
     ...
     @endif
				...
				<form class="mt-4" id="add_prog_form" method="post" action="{{ action('TeacherController@store') }}">
        {{csrf_field()}}
        <div class="form-group">
      <input type="text" name="staff_code" placeholder="Staff ode" />
    </div>
        <div class="form-group">
		...
		<table class="table table-striped mb-0">
		  <thead>
		    <tr>
		      <th scope="col">Staff Code<i class="fas fa-sort-down"></i></th>
 ...
 <td>{{$faculty->staff_code}}</td>
  ...
  <!-- <td>{{$faculty->address}}</td> -->
		      ...
		      <td>
          <form action="{{ route('teachers.destroy',$faculty->id) }}" method="POST">
		  @csrf
            @method('DELETE')
			<a class="green" href="#"><i class="far fa-eye mr-3"></i></a>
		    <a class="green"href="{{ route('teachers.edit', $faculty->id) }}"><i class="fas fa-pen mr-3"></i></a>
           <a class="red" onclick="return confirm('Are you sure?')" ><i class="fas fa-trash-alt"></i></button>
		...
		<div class="tbl-pagination mt--20">
			<ul class="pagination">
			  <!-- <li class="page-item"><a  href="#"><i class="fas fa-arrow-left"></i></a></li> -->
			  <li class="page-item">{{$facultys}}</li>
